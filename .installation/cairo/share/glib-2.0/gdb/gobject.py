...
def g_type_to_name(gtype):
    def lookup_fundamental_type(typenode):
        ...
        return None if val is None else val[typenode >> 2].address()
    ...
    return glib.g_quark_to_string (typenode["qname"]) if typenode != None else None
...
def get_signal_name(id):
    ...
    max_s = long(max_s)
    return val[id]["name"].string() if id < max_s else None
...
class GFrameWrapper:
    ...
    def name(self):
        ...
        return name[4:] if name and name.startswith("IA__") else name
...
class SignalFrame:
    ...
    def or_join_array(self, array):
        ...
        v = {str(array[i]): 1 for i in range(len(array))}
        array = v.keys()
        s = array[0]
...
class GFrameFilter:
    ...
    def next(self):
        ...
        if emission > 0:
            ...
            new_frames = [
                DummyFrame(signal_frames[i]) for i in range(len(signal_frames) - 1)
            ]
